...
export async function before(m) {
  ...
  if (!(id in this.emailotp)) {
    return this.reply(m.chat, "*OTP ini telah berakhir!*", m)
  }
...
ᴅᴀᴛᴀ ᴜsᴇʀ ʏᴀɴɢ ᴛᴇʀsɪᴍᴘᴀɴ ᴅɪᴅᴀᴛᴀʙᴀsᴇ ʙᴏᴛ, ᴅɪᴊᴀᴍɪɴ ᴀᴍᴀɴ ᴛᴀɴᴘᴀ ᴛᴇʀsʜᴀʀᴇ (. ❛ ᴗ ❛.)`
        await conn.reply(m.chat, cap, m, { contextInfo: { mentionedJid: [who], forwardingScore: 9999, isForwarded: true, externalAdReply: { mediaType: 1, mediaUrl: pp, title: `sᴇʟᴀᴍᴀᴛ +${this.emailotp[id][2]} xᴘ`, thumbnail: { url: pp }, thumbnailUrl: pp, sourceUrl: false, renderLargerThumbnail: true }}})
        clearTimeout(this.emailotp[id][3])
        delete this.emailotp[id]
